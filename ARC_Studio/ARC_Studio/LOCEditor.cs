...
namespace ARC_Studio.ARC
{
    public partial class LOCEditor : Form
    {
        ...
        {
            base.OnLoad(e);
            RPC.SetLocEditorPresence();
        }

        protected override void OnActivated(EventArgs e)
        {
            base.OnActivated(e);
            RPC.SetLocEditorPresence(openedloc);
        }
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            switch (keyData)
            {
                case (Keys.Control | Keys.Z):
                    sameToolStripMenuItem_Click(null, null);
                    return true;
                default:
        ...
        private void sameToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (this.languagesContainer_0 != null)
            {
                if (!string.IsNullOrWhiteSpace(openedloc))
                {
                    ...
                    Messenger.MessageBox("Languages save has completed.", "Save completed", MessageBoxButtons.OK, MessageBoxIcon.None);
                }
            }
        }
