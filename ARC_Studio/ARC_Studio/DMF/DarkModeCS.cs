...
namespace DarkModeForms
{
	...
	public class DarkModeCS
	{
		...
		public void ThemeControl(Control control)
		{
			try
			{
				...
				{
					control.GetType().GetProperty("BackColor")?.SetValue(control, control.Parent.BackgroundImage);
					control.GetType().GetProperty("ForeColor")?.SetValue(control, OScolors.Accent);
					control.GetType().GetProperty("BorderStyle")?.SetValue(control, BorderStyle.None);
				}
				...
				if (control is MenuStrip)
				{
					...
					(control as MenuStrip).Renderer = new MyRenderer(new CustomColorTable(OScolors))
					{
						MyColors = OScolors
					};
				}
	...
	public class MyRenderer : ToolStripProfessionalRenderer
	{
		...
		public MyRenderer(ProfessionalColorTable table, bool pColorizeIcons = false) : base(table)
		{
