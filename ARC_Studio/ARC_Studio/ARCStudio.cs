...
namespace ARC_Studio
{
    ...
    public partial class ARCStudio : Form
    {
        public ARCStudio()
        {
            ...
            #region ImageViewer
            var basePic = pictureBoxWithInterpolationMode2;

            ImageViewer viewer = new ImageViewer()
            {
                Name = "animatedViewer",
                Location = basePic.Location,
                Size = basePic.Size,
                MaximumSize = basePic.MaximumSize,
                Anchor = basePic.Anchor,
                Visible = false
            };
        ...
        {
            Messenger.MessageBox("ARC Studio Remake v1.9\n\nDeveloper/Designer: DescZ\nHelper/Designer: EternalModz\nSource code: PhoenixARC", "Credits", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private async void saveToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            using (var loading = new LoadingForm("Saving file..."))
            {
                try
                {
                    loading.Show();
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            e.Graphics.DrawRectangle(pen, new Rectangle(0, 3, Width - 3, Height - 3));
        }
        ...
        private void EntryList_AfterSelect(object sender, TreeViewEventArgs e)
        {
            try
            {
                ...
                if (ext == ".png" || ext == ".jpg")
                {
                    var viewer = this.Controls.Find("animatedViewer", true).FirstOrDefault() as ImageViewer;

                    if (viewer != null)
                    {
                        viewer.Visible = true;

                        using (FileStream fs = new FileStream(selectedPath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                        using (Image img = Image.FromStream(fs))
                        {
                            viewer.SetImage(new Bitmap(img));
                            SizeLabel1.Text = $"{img.Width} x {img.Height}";
                        }
                    }
                    ...
                    pictureBoxWithInterpolationMode2.InterpolationMode = InterpolationMode.HighQualityBicubic;

                    using (FileStream fs = new FileStream(selectedPath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                    using (Image skinPicture = Image.FromStream(fs))
                    {
                        if (loadedImage != null)
                            loadedImage.Dispose(); // ensure old one is freed

                        loadedImage = new Bitmap(skinPicture);
                        pictureBoxWithInterpolationMode2.Image?.Dispose();
                        pictureBoxWithInterpolationMode2.Image = new Bitmap(loadedImage);
                    }
